...
if ($tipo == "tutti") {
    ...
    $where = "WHERE stato_pren = 'ATTESA ACCETTAZIONE' AND tipo_pren = 'PRIVATO'";

}elseif ($tipo == "lavorazione") {
    $where = "WHERE stato_pren = 'LAVORAZIONE' AND tipo_pren = 'PRIVATO'";

}elseif ($tipo == "terminato") {
    $where = "WHERE stato_pren = 'TERMINATO' AND tipo_pren = 'PRIVATO'";
}
...
echo "<div class ='row'><p class='btn btn-danger center-button'>Non ci sono appuntamenti in $tipo di Clienti Privati nel database.</p></div>";
echo "</div>";
