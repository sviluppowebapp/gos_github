...
$sq1 = "SELECT id_app FROM appuntamenti WHERE stato_pren = 'TERMINATO'";
...
echo "<div class ='row'><p class='btn btn-warning center-button'>Non ci sono appuntamenti terminati</p></div>";
echo "</div>";
...
$sq2 = "SELECT * FROM appuntamenti WHERE stato_pren = 'TERMINATO' ORDER BY data DESC LIMIT $first, $x_pag";
...
<td class='table-success-rs'><? echo $stato_pren; ?></td>
<td><? echo $note; ?></td>
...
if ($all_pages > 1){
  if ($pag > 1){
    echo "<a href=\"" . $_SERVER['PHP_SELF'] . "?page=appuntamenti_terminati&pag=" . ($pag - 1) . "\">";
    echo "&lt;&lt; Precedente</a>&nbsp;&nbsp;&nbsp;";
  } 
  if ($all_pages > $pag){
    echo "<a href=\"" . $_SERVER['PHP_SELF'] . "?page=appuntamenti_terminati&pag=" . ($pag + 1) . "\">";
